...
float4 PS_Main(PixelInputType input) : SV_TARGET0
{
	...
	S = saturate(S);
	//スペキュラーカラーを加算する
	float4 Out = diffuseColor * shaderTexture.Sample(SampleTypeWrap, input.tex) + float4(input.normal2.xyz,1);
