...
bool ColorVertexShader::Setup()
{
	...
	(
		m_cpVertexShader,
		_T("Resource/Shader/HLSL/ColorSprite.hlsl"),
		"VS_Main",
...
void ColorVertexShader::Destroy()
{
}

bool ColorVertexShader::PreProcessOfRender(std::shared_ptr<SubMesh> shape, std::shared_ptr<Material>materials)
{
	HRESULT hr;
	hr = E_FAIL;
	...
	m_indexCount = shape->GetIndexBuffer()->GetDataCount();
	sRENDER_DEVICE_MANAGER->SetBackBufferRenderTarget();
	return hr;
}


bool ColorVertexShader::Render()
{
	HRESULT hr;
	hr = E_FAIL;
...
bool ColorVertexShader::PostProcessOfRender()
{
	HRESULT hr;
	hr = E_FAIL;
